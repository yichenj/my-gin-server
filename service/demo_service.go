...
func (service *DemoService) Create(demoDTO *dto.DemoDTO) (int, error) {
  applog.Info.Printf("This extra info: \"%s\" is not useful at all " +
                     "but to explain why we need a dto sometimes.", demoDTO.ExtraInfo)
  demo := &model.Demo{Name:demoDTO.Name, Description:demoDTO.Description, CreateTime:time.Now().UTC()}
...
func (service *DemoService) DeleteRange(offset int, limit int) (int, error) {
  ...
  // Transactions always involve many DAOs, so usually we need a transaction creation at
  // the service level.
...
func (service *DemoService) List(offset int, limit int) ([]*model.Demo, int, error) {
